...
%% good file read
good_file = 'C:\Users\qotng\Desktop\GPG\HCI_논문준비\매틀랩 준비\매틀랩코드\good_exel.xlsx';
...
%% moderate file read
moderate_file = 'C:\Users\qotng\Desktop\GPG\HCI_논문준비\매틀랩 준비\매틀랩코드\moderate4_exel.xlsx';
...
%% poor file read
poor_file = 'C:\Users\qotng\Desktop\GPG\HCI_논문준비\매틀랩 준비\매틀랩코드\poor5_exel.xlsx';
...
slow_file = 'C:\Users\qotng\Desktop\GPG\HCI_논문준비\매틀랩 준비\매틀랩코드\slow5_exel.xlsx';
...
for i = 31:40
 ...
 subplot(1,3,1);
 hold on
 bar(label, tm_mean, 0.5) 
 for i = 1:10
     eval(['plot(label(1),A_tm' num2str(i,'%02d') ',' '''--+r'')']);
 end
  for i = 11:20
     eval(['plot(label(2),A_tm' num2str(i,'%02d') ',' '''--+r'')']);
  end
  for i = 21:30
     eval(['plot(label(3),A_tm' num2str(i,'%02d') ',' '''--+r'')']);
  end 
%   for i = 31:40
...
%   end 
 ...
 subplot(1,3,2);
 hold on
 bar(label, peak_mean, 0.5)
 for i = 1:10
     eval(['plot(label(1),B_peak' num2str(i,'%02d') ',' '''--+r'')']);
 end
   for i = 11:20
     eval(['plot(label(2),B_peak' num2str(i,'%02d') ',' '''--+r'')']);
  end
  for i = 21:30
     eval(['plot(label(3),B_peak' num2str(i,'%02d') ',' '''--+r'')']);
  end
%   for i = 31:40
...
%   end
 ...
 hold off
  ...
  subplot(1,3,3);
 hold on
 bar(label, err_dis_mean, 0.5)
 for i = 1:10
     eval(['plot(label(1),C_error_dis' num2str(i,'%02d') ',' '''--+r'')']);
 end
   for i = 11:20
     eval(['plot(label(2),C_error_dis' num2str(i,'%02d') ',' '''--+r'')']);
  end
  for i = 21:30
     eval(['plot(label(3),C_error_dis' num2str(i,'%02d') ',' '''--+r'')']);
  end
%   for i = 31:40
...
subplot(2,2,1);
 hold on
 bar(label_v, peak_mean_v, 0.5)
  for i = 21:30
     eval(['plot(label_v(1),B_peak' num2str(i,'%02d') ',' '''--+r '')']);
  end
  for i = 31:40
     eval(['plot(label_v(2),B_peak' num2str(i,'%02d') ',' '''--+r'')']);
  end
 ylabel('Hz');
xticks([]);
...
subplot(2,2,2);
 hold on
 bar(label_v, tm_mean_v, 0.5) 

  for i = 21:30
     eval(['plot(label_v(1),A_tm' num2str(i,'%02d') ',' '''--+r'')']);
  end 
  for i = 31:40
     eval(['plot(label_v(2),A_tm' num2str(i,'%02d') ',' '''--+r'')']);
  end 
 ylabel('cm');
 xticks([]);
subplot(2,2,3);
 hold on
 bar(label_v, time_mean_v, 0.5) 
  for i = 21:30
     eval(['plot(label_v(1),D_time' num2str(i,'%02d') ',' '''--+r'')']);
  end 
  for i = 31:40
     eval(['plot(label_v(2),D_time' num2str(i,'%02d') ',' '''--+r'')']);
  end 
   hold off
 ylabel('sec');
 xticks([]);
  ...
  subplot(2,2,4);
 hold on
 bar(label_v, velocity_mean_v, 0.5)
  for i = 21:30
     eval(['plot(label_v(1),E_velocity' num2str(i,'%02d') ',' '''--+r'')']);
  end
  for i = 31:40
     eval(['plot(label_v(2),E_velocity' num2str(i,'%02d') ',' '''--+r'')']);
  end
 ylabel('cm/sec');
 xticks([]);
 hold off
 ...
 peak_p4 = ranksum(peak_slow, peak_poor);
 AA_peak=[0,0,0,0];
 if(peak_p1 <=0.05)
    AA_peak(1,1) =1;  
 end
...
AA_error_dis=[0,0,0];
 ...
 velocity_p3 = ranksum(velocity_poor,velocity_slow);
AA_velocity=[0,0,0];
 if(velocity_p1 <=0.05)
    AA_velocity(1,1) =1;  
 end
...
axis equal
hold on 
plot(moderate_yp16,moderate_xp16)
hold off
% title('Moderate');
xticks([]);
yticks([]);
...
plot(base_l(1,1:511))
hold on 
plot(good_time06/1000,good_xp06)
hold off
% xlabel('time(msec)');
xlim([1 8.4]);
ylim([0 990])
...
plot(base_l(1,1:end))
hold on 
plot(moderate_time16/1000,moderate_xp16)
hold off
% xlabel('time(msec)');
xlim([1 4.8]);
ylim([0 990])
...
plot(base_l(1,1:end))
hold on 
plot(poor_time26/1000,poor_xp26)
hold off
% xlabel('time(msec)');
% xticks([]);
xlim([1 8.7]);
...
plot(base_l(1,1:end))
hold on 
plot(slow_time36/1000,slow_xp36)
hold off
% xlabel('time(msec)');
xlim([1 35]);
ylim([0 990])
...
hold on 
% plot(pca_sig06);
plot(good_time06(1,5:end-5)/1000,low1(1,5:end-5));
hold off
xlim([1 8.4]);
ylim([0 990])
ylim([-250 1450]);
...
hold on 
% plot(pca_sig16);
plot(moderate_time16(1,5:end-5)/1000,low2(1,5:end-5))
hold off
xlim([1 4.8]);
ylim([0 990])
yticks([]);
...
plot(poor_time26(1,5:end-5)/1000,low3(1,5:end-5))
hold off
ylim([-250 1450]);
yticks([]);
% xticks([]);
...
plot(slow_time36(1,5:end-5)/1000,low4(1,5:end-5))
hold off
xlim([1 35]);
ylim([0 990])
yticks([]);
...
% for i = 31:40
%     subplot(4,3,i-30)
%     eval(['plot(fft_freq' num2str(i,'%02d') ',fft_result' num2str(i,'%02d') ');']);
%     hold on 
%     eval(['plot(fft_limitline' num2str(i,'%02d') ');']);
%     hold off
%     xlim([1 30]);
%     ylim([0 100]);
% end